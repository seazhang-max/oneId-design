...
| 阶段  | 名称           | 核心产出           | 执行频率  | 技术                |
| --- | ------------ | -------------- | ----- | ----------------- |
| 1   | 数据采集与标准化清洗   | 标准化点表 + 边表     | 每日/实时 | spark/bigquery    |
...
| 6   | 跨层映射表计算与向下传递 | 跨层映射表 + 合并建议   | 每日批处理 | spark             |
| 7   | 服务发布与缓存预热    | 在线服务就绪         | 每日批处理 | bigquery          |
| 8   | 全量重算比对与纠错    | 校验报告 + 纠错后的 OneID | 每周/每月 | spark             |
| 9   | 人工干预 OneID 合并拆分 | OneID 映射关系即时更新 + AI 属性 1 小时内增量计算完成 | 按需触发 | spark/flink/redis |
...
**性能优化原则 — 能用匹配关联就不用 WCC 重算**：

WCC 是图计算中最耗性能的操作（时间复杂度 O(E × k)），因此在增量场景下，应优先使用轻量级的匹配关联来处理，只有在匹配关联无法解决时才启用 WCC 重算：


| 增量操作类型     | 优先处理方式                                    | 何时需要 WCC 重算                                   |
| ---------- | ----------------------------------------- | --------------------------------------------- |
| **新增顶点/边** | 通过 ID 标识匹配历史 OneID，直接归属（阶段 4.1 的 JOIN 操作） | 仅当新增边导致多个历史 OneID 连通时，才需要局部 WCC 确认融合范围        |
| **删除顶点**   | 直接从 H_old 成员列表中移除，无需 WCC                  | 仅当被删除顶点是**桥接点**（即移除后子图可能断裂）时，才需要局部 WCC 判定是否分裂 |
| **删除边**    | 如果两端顶点仍通过其他路径连通，保持不变                      | 仅当无法确认两端顶点是否仍连通时，才需要局部 WCC                    |
| **修改顶点属性** | 直接更新 H_old 的属性字段，无需 WCC                   | 不涉及图结构变化，永远不需要 WCC                            |


简而言之：**匹配关联是 O(1) 的哈希查找，WCC 是 O(E × k) 的图遍历**。在增量场景下，绝大多数操作（新增归属、删除成员、属性更新）都可以通过匹配关联完成，只有涉及图拓扑结构变化（桥接点删除导致潜在分裂、新增边导致潜在融合）时，才不得不动用局部 WCC 重算。这也是为什么"删除点后局部重算"被单独列为一种场景——它的触发条件是"删除后子图可能断裂"，如果能通过其他手段（如维护桥接点索引）提前判断不会断裂，就可以跳过 WCC。
...
- 如果一个原始 UID 的绑定标识分散在多个 OneID 中（跨分量冲突），按 4.5 的冲突消解规则处理
...
在 4.2~4.6 的决策过程中，每当原始业务 ID 的 OneID 归属发生变化时，都需要记录一条变更流水（对应 BigQuery 表 `dws_oneid_identity_original_id_change_di`）。该流水表用于回答"某个 UID 的 OneID 是如何变化的"。
...
```
对于每个在 4.2~4.6 中发生归属变化的原始业务 ID 绑定记录 B：
  生成变更事件：
    change_id = UUID
    change_type = 根据上表映射
    scope = 当前层级
...
将阶段 5 产出的 OneID 宽表和阶段 6 产出的跨层映射表发布到各类存储系统，支撑上层业务的实时查询和分析需求。

### 输入

- 通过质量校验的 OneID 宽表
- 跨层映射表（CrossLayerMapping，阶段 6 产出）
...
增量更新在长期运行中可能因累积误差导致结果偏离正确值（如边的增删顺序不同可能导致连通分量结果差异）。本阶段通过**定期执行全量重算**，将全量计算结果与增量累积结果进行比对，发现差异时告警并采用全量计算结果作为权威基准，确保 OneID 体系的长期准确性。

### 输入

- 当前增量累积的 OneID 库（阶段 4 产出）
- 完整的点表和边表（阶段 1 产出）
- 各层 OneID 的 ID 标识列表
...
| 触发方式     | 触发条件                                                | 频率            |
| -------- | --------------------------------------------------- | ------------- |
| **定期触发**   | 按调度计划自动执行（如每周日凌晨）                                    | 每周/每月         |
| **阈值触发**   | 增量更新累积的融合/分裂操作数量超过总 OneID 数量的 N%（如 5%）             | 每轮增量后检查       |
| **手动触发**   | 运维人员怀疑数据质量问题时手动发起                                   | 不定期           |
| **重大变更触发** | 数据源结构变化、边权重规则调整、超级节点阈值修改等配置变更后自动触发                  | 配置变更时         |
...
```
差异率 = 归属发生变化的顶点数 / 全图顶点总数 × 100%

其中"归属发生变化的顶点"指：
  在全量结果中属于分量 A，但在增量结果中属于分量 B（A ≠ B）的顶点
```

#### 8.4 安全保障机制

- **双写机制**：全量重算结果先写入临时表，比对完成并确认后才切换为正式结果，避免中间态影响在线服务
- **回滚能力**：保留切换前的增量 OneID 库快照，如果全量结果反而有问题（如数据源本身出错），可快速回滚
- **幂等性**：全量重算的结果不依赖历史 OneID 库，因此多次执行结果一致（同一份输入数据 → 同一份输出结果）

### 输出

- 全量重算比对报告（差异类型、差异率、受影响的 OneID 列表）
- 告警通知（INFO/WARNING/CRITICAL 级别）
- 纠错后的 OneID 库（以全量计算结果为权威基准）
- 纠错审计日志
...
```
增量 OneID 库当前状态：
  H1 = {phone:138, email:a@x.com, openid:ox_abc}
  H2 = {phone:139, email:b@x.com}

全量 WCC 结果：
  C1 = {phone:138, email:a@x.com}
  C2 = {phone:139, email:b@x.com, openid:ox_abc}

比对发现：
  openid:ox_abc 在增量库中属于 H1，在全量结果中属于 C2
  → 差异类型 C（成员偏移）
  → 根因：增量过程中新增了一条 edge(openid:ox_abc, phone:139)
    但增量更新未触发 H1 的重新计算
  
差异率 = 1/5 = 20%（5 个顶点中 1 个归属变化）
→ CRITICAL 告警
→ 采用全量结果，H1 和 H2 的成员列表按 C1、C2 重新调整
```
...
| 层级  | 重算范围              | 比对对象                    |
| --- | ----------------- | ----------------------- |
| 品牌层 | 该品牌下的完整点表和边表     | 品牌层增量 OneID 库           |
| 集团层 | 该集团下所有品牌的完整点表和边表 | 集团层增量 OneID 库           |
| 全局层 | 所有集团的完整点表和边表     | 全局层增量 OneID 库           |
...
品牌层、集团层、全局层各自独立执行阶段 1~5，互不依赖。阶段 6（跨层映射表）在所有层的阶段 5 完成后执行，阶段 7（服务发布）在阶段 6 完成后执行，阶段 8（全量重算比对）按需定期执行，阶段 9（人工干预）随时按需触发：
...
- 阶段 1~5 可以完全并行计算，无数据依赖
- 阶段 6（跨层映射表）必须在所有层的阶段 5 完成后执行（依赖各层 OneID 结果）
- 阶段 7（服务发布）必须在阶段 6 完成后执行（依赖跨层映射表）
- 阶段 8（全量重算比对）定期或在阈值触发时执行，三层各自独立重算并与增量结果比对，发现差异时以全量结果为准
- 阶段 9（人工干预）随时按需触发，OneID 映射关系立即生效，AI 属性在 1 小时内增量计算完成
- 各层对"同一个人"的判定可能不同（因为数据范围不同，连通分量结果不同），这是预期行为
- 跨层映射表通过向下传递机制，为集团层和品牌层的 OneID 合并提供决策依据

---

## 阶段 9：人工干预 OneID 合并拆分机制
...
人工干预操作采用**双阶段生效**策略，确保 OneID 映射关系的即时可用性与 AI 属性的最终一致性：

| 阶段 | 生效内容 | 生效时间 | 说明 |
| --- | --- | --- | --- |
| **即时生效** | OneID 映射关系变更（合并/拆分/解绑）+ 干预规则同步 | 页面操作后**立即生效** | 直接更新 OneID 映射表、跨层映射表、Redis/HBase 缓存；同时**将干预规则同步写入 Spark 可读取的干预规则表**，确保后续离线批处理（阶段 1~4）不会覆盖人工干预的结果 |
| **延迟计算** | AI 属性（AIProfile）更新 | 操作后**1 小时内**完成增量计算 | 触发 Flink 增量任务，重新计算受影响 OneID 的 AI 属性（生命周期、偏好模型、流失概率等），完成后更新存储 |
...
- **即时生效**：OneID 映射关系在页面操作后立即生效，确保业务系统可立即使用最新的身份归属
- **最终一致**：AI 属性通过增量计算在 1 小时内完成更新，期间下游系统可读取到旧的 AI 属性或标记为"recalculating"状态
