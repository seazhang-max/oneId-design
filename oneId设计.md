...
**// 文档太长了，把oneid定义+三层架构+合并/拆分/覆盖的触发场景说明、实时&离线的场景说明，异常场景（换手机号、用户注销后系统如何处理oneid）抽出来做一个简略的文档是不是更好**
...
### 数据结构 **// 直接进入数据结构，其他人不容易理解，建议在这个前面补充一个oneid--mapping--物理属性等的关联关系的总图**
...
OneID {
  ...
  status: Enum,                // 状态: "active" | "merged" | "split" **// 应该包括delete**
...
CrossLayerMapping {
  ...
  propagation_direction: Enum,     // 传递方向: "downstream" (向下) | "upstream" (向上) **// 如果是每层oneid从边表开始独立计算，应该不存在向上传递的方向**
...
| 2   | 图数据加载与预处理    | 已缓存的干净图 // 干净图这个名词很突兀，下面也没有详细说明       | 每日批处理 | spark             |
...
- 业务系统提供的用户 ID 标识数据：手机号、邮箱、设备 ID、OpenID、UnionID、Cookie ID、第三方 ID 等（每天增量）// 不明白unionid是什么
...
- **测试数据**：标记为测试的账号、明显测试手机号（如 13800000000）//感觉这里会被挑战说：1.为什么测试数据会进生成系统； 2.假如1.是允许的，为什么不让测试账号也参与oneid聚合，是否可以人工控制测试账号参与聚合
...
OriginalIdBinding {
  ...
  original_id_type: String,    // 原始 ID 类型: ecommerce_uid | app_uid | wechat_uid | external_uid | account_id // 没有注释
  ...
  // 绑定质量
  binding_confidence: Float,   // 绑定置信度（直接绑定=1.0，间接推断<1.0）// 间接推断是什么，对应绑定来源的login_binding和event_inference? 这个字段的意义不太明白，是给边赋予置信度？这几张表最好也有个数据流动的流程图说明
...
- 软剪枝（可选）：保留超级节点本身，但降低其关联边的权重至接近 0 **// 这个策略感觉没必要，直接硬剪枝更好**
- 被剪枝的超级节点单独记录，供风控模块使用（可能是羊毛党/欺诈信号）

#### 2.3 删除点与删除边的增量处理

当点表或边表中存在 `status = "deleted"` 的记录时，需要在构建 GraphFrame 后执行增量处理，确保删除操作正确反映到图结构中：
**// 增量计算输入点和边，受影响的连通分量在全部连通分量计算完成后再用局部wcc寻找，那实际上每次增量在图计算也要跑一次全部点和边的计算，没有之前点+边+受影响的连通分量去跑增量合理 **
**// 而且文字这样的组织要到局部wcc才能看出全量和增量的区别 **
**删除顶点的处理**：
...
**// 这里我还是不太明白，如果我们把数据加载&图计算&图算结果落存储用同一个spark任务去跑，为什么会要从action多次加载。 **
...
- **新增边导致图结构变化**：新的 ID 标识将原本连通的顶点分隔到不同分量 **// 新增应该只会合并连通分量，新增导致分割不太明白，最好用案例说明**
- **删除点/边导致连通性破坏**：阶段 2 中的删除操作（如脏数据清理、用户注销、人工干预）导致原本连通的顶点不再连通
- **数据修正**：历史数据的错误被修正后，导致图结构重新计算

无论触发原因如何，阶段 4 的判定逻辑一致：只要 H_old 的顶点分散到多个 C_new，就执行分裂操作。**//如何判断或者说区分出h_old的顶点有被删除几个，和c_new这轮算法有问题，导致生成的c_new们丢失了部分h_old顶点这两种情况 **
...
向下查询：
  ...
  合并建议：GR:G1:001 和 GR:G2:001 应该合并 **// 不是同一个集团的两个oneid，没必要建议合并吧？建议应该给到同一个集团下对应同一个真实用户的多个账号**
```
...
### 处理流程
**// 下图生成临时uuid，一天内多次的实时计算，会导致同一个人生成多个临时uuid吧，这如何处理？**
```
用户事件（注册/登录/浏览/下单）
       │
       ▼
...
品牌层、集团层、全局层各自独立执行阶段 1~5，互不依赖。阶段 6（跨层映射表）在所有层的阶段 5 完成后执行，阶段 7（服务发布）在阶段 6 完成后执行：
