...
**这里的概念组织建议细化表明：
OneID 不是简单等于一个自然人。
OneID 是系统在某个数据范围内，对“同一用户身份”的判断结果。
品牌层只在一个品牌内判断同一个人。
集团层在一个集团内跨品牌判断同一个人。
全局层在更大范围内判断可能的同一个人。**
...
OneID {
  ...
  batch_id: String,            // 生成/更新该 OneID 的计算批次号 --有这个还需要上面的版本号吗
...
#### 跨层映射表（独立存储）--这个表在这里突然出现，不看下面不太明白
...
**流水线这部分太多技术细节了，建议补上例子说明业务用户如何映射到 OneID这条数据流**
...
**cache、checkpoint这些图计算细节感觉可以不用说**
...
为每个确定的 OneID 聚合来自多源的用户属性，构建完整的用户画像（PhysicalInfo + AIProfile）。-- 每次聚合都要重算吗，还是会继承旧oneid的一部分再增量计算
...
#### 6.1 多存储引擎写入 -- 感觉这里会被问为什么需要那么多的异构存储
...
**看到这里也似乎没有描述出找出全局oneid的作用，比如是更新下游的画像，还是增值服务向下传递给品牌和集团**
